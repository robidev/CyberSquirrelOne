...
public partial class ResizableUI : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
	...
	[NonSerialized] public MenuHelper menuHelper;
	...
	public virtual void MaximizeToCanvas()
	{
		//Vector3 pos = rect.anchoredPosition; pos.x =0; pos.y =0;
		RectTransform menuRect = menuHelper.targetMenu.GetComponent<RectTransform> ();

		Vector3 pos = rect.anchoredPosition;
		pos.x = -canvas.pixelRect.width / 2;
		pos.y = menuRect.rect.height - canvas.pixelRect.height / 2;
		rect.anchoredPosition = pos;

		rect.sizeDelta = new Vector2(canvas.pixelRect.size.x, canvas.pixelRect.size.y - menuRect.rect.height);
	}
	...
	protected virtual void Awake()
	{
		...
		menuHelper = GetComponent<MenuHelper> ();
	}
